...
namespace std
{
	...
	void Swap(short& Number1, short& Number2)
	{
		...
		//ANOTHER SOLUTION (BUT IT TAKES MORE SPACE)
	...
	void Swap(long long& Number1, long long& Number2)
	{
		Number2 = Number1 + Number2;
		Number1 = Number2 - Number1;
		Number2 = Number2 - Number1;
	...
	void Swap(long double& Number1, long double& Number2)
	{
	...
	void Swap(bool& Boolean1, bool& Boolean2)
	{
	...
	long long ReadIntegerPositiveNumber(string Message)
	{
		ios_base::sync_with_stdio(false);
		cin.tie(NULL);
	...
	char ReadArithmeticOperator(string Message)
	{
		ios_base::sync_with_stdio(false);
		cin.tie(NULL);
		...
		do
		{
			Operation = ReadCharacter(Message);
	...
	void FillArrayWithRandomNumbers(int Array[100], int& ArraySize, int From, int To)
	{
		ArraySize = ReadPositiveNumber("ENTER ARRAY SIZE");
	...
	void CopyArrayInReversedOrder(int CopiedArray[], int ArrayReversedCopy[], int CopiedArraySize)
	{
		for (int i = 0; i < CopiedArraySize; i++)
		{
	...
	void FillMatrixWithRandomNumbers(int Matrix[100][100], int Rows, int Columns, int From, int To)
	{
		for (int i = 0; i < Rows; i++)
		{
			for (int j = 0; j < Columns; j++)
	...
	int MatrixSum(int Matrix[100][100], int Rows, int Columns)
	{
		int MatrixSum = 0;
		for (int i = 0; i < Rows; i++)
		{
	...
	string Alarm(short NumberOfAlarms)
	{
		string Alarm = "";
	...
	short CharacterFrequencyInString(string String, char TargetedCharacter)
	{
		short CharacterFrequency = 0;

		for (int i = 0; i < String.length(); i++)
		{
			(String[i] == TargetedCharacter) ? CharacterFrequency++ : CharacterFrequency;
		}
	...
	vector<string> SplitStringWordsToVector(string String, string Delimiter)
	{
		short DelimiterPosition = 0;
		string Word = "";
	...
	string JoinStringsFromArrayInReversedOrder(string StringArray[100], int ArrayLength, string Separator)
	{
		string String = "";
	...
	{
		vector<string>StringVector = SplitStringWordsToVector(String, " ");
	...
	string ReplaceCharactersInString(string String, char ReplacedCharacter, char ReplacingCharacter)
	{
	...
	string ReplaceLettersInString(string String, char ReplacedLetter, char ReplacingLetter, bool MatchCase = true)
	{
		for (char& Character : String)
		{
			if (MatchCase)
	...
	string RemoveLetterFromString(string String, char TargetedLetter , bool MatchCase = true)
	{
		string FilteredString = "";
	...
	string RemoveWordsFromString(string String, string RemovedWord , bool MatchCase = true)
	{
		vector<string>StringVector = SplitStringWordsToVector(String, " ");
	...
	bool IsDate1BeforeDate2(Date Date1, Date Date2)
{
	...
	long long CalculateEndOfTheWeekDays(Date StartDate, Date EndDate)
	{
	...
	long long CalculateWeekEndDays(Date StartDate, Date EndDate)
	{
	...
	long long CalculateBusinessDays(Date StartDate, Date EndDate)
	{
	...
	Date CalculateVacationStartDate(Date VacationEndDate, long long VacationDays)
	{
	...
	CompareDatesResults CompareDates(Date Date1, Date Date2)
	{
	...
	void Swap(Period Period1, Period Period2)
	{
	...
	{
		vector<string>DateVector = SplitStringWordsToVector(DateString, "/");
	...
	{
		vector<string>SystemThemeColorsVector = SplitStringWordsToVector(Record, Separator);
